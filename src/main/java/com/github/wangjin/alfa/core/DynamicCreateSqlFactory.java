...
import com.github.wangjin.alfa.constant.MapperConstants;
import com.github.wangjin.alfa.helper.CommonsHelper;
import com.github.wangjin.alfa.helper.DynamicQuerySqlHelper;
...
public final class DynamicCreateSqlFactory {
     ...
     * @return sql
     */
    public String getInsertListSql() {
        StringBuilder sqlBuilder = new StringBuilder();
        sqlBuilder.append("<script>");
        ...
        sqlBuilder.append(Arrays.stream(providerTable.getColumns())
                .map(ProviderColumn::getDbInsertColumn).collect(Collectors.joining(",")));
        sqlBuilder.append(") values ");
        sqlBuilder.append("<foreach collection=\"list\" separator=\",\" item=\"item\">(");
        sqlBuilder.append(Arrays.stream(providerTable.getColumns())
                .map(ProviderColumn::getJavaColumn)
                .map(a -> CommonsHelper.format("#{item.%s}", a))
        ...
        if (insertSelective) {
            ...
            for (ProviderColumn column : providerTable.getColumns()) {
                ...
                sqlBuilder.append(CommonsHelper.format("<if test=\"%s != null %s\">%s,</if>",
                        column.getJavaColumn(), stringIf, column.getDbInsertColumn()));
            }
